...
class Installer(ABC):
    ...
    def __init__(self):
        ...
        self.last_error_callback = None
        self.complete_callback = None

    def register_progress_callback(self, progress_callback):
        self.progress_callback = progress_callback

    def register_last_error_callback(self, last_error_callback):
        self.last_error_callback = last_error_callback

    def register_complete_callback(self, complete_callback):
...
class GenericDDIClient(object):
    ...
    def __init__(self, session, host, ssl, tenant_id, target_name, auth_token,
                 attributes, bundle_dl_location, installer: Installer, result_callback, step_callback=None,
                 lock_keeper=None):
        ...
        self.installer.register_progress_callback(self.progress_callback)
        self.installer.register_last_error_callback(self.last_error_callback)
        self.installer.register_complete_callback(self.complete_callback)
    ...
    async def progress_callback(self, percentage, description):
    ...
    async def install(self, action_id, filepath):
    ...
    async def process_deployment(self, base):
        ...
        try:
            # TODO currently only first chunk ("artifact") is downloaded
        ...
        try:
            ...
            await asyncio.shield(self.install(action_id, filepath))
        except:
            # send negative feedback to HawkBit
            status_execution = DeploymentStatusExecution.closed
            status_result = DeploymentStatusResult.failure
            await self.ddi.deploymentBase[action_id].feedback(
                status_execution, status_result, [])
            raise APIError()

    async def download_artifact(self, action_id, filename, url, md5sum,
                                tries=3):
